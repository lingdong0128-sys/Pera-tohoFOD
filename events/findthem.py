def event_findthem(self):
    ...
    while True:
        ...
        if input=='5':
            asking = True
            while asking:
                self.console.PRINT("「不知不觉间这个世界已经在逐渐丰满起来了，我能感觉到一种神秘的“规则”正在形成，那可能是一种控制生灵说话的法则...好像是叫口上！」",colors=(0, 188, 212))
                self.console.PRINT("「不过这些都是我猜测的，我并不清楚具体内容....」",colors=(0, 188, 212))
                ...
                if choice=='0':
                    ...
                    self.console.PRINT("「那么你觉得这个世界是什么样子的呢？」",colors=(0, 188, 212))
                    self.console.PRINT("[0]「世界就是世界啊...」",click="0")
                    self.console.PRINT("[1]「世界是一个巨大的草莓蛋糕！」",click='1')
                    self.console.PRINT("[2]「世界是存在于许多世界中的一个小小的个体，而我们是小小个体中的小小个体」",click='2')
                    self.console.PRINT("[3]「世界是一串串python事件组成的！我们都是数据！！！！这是一个事件哈哈哈！」",click='3')
                    self.console.PRINT("[4]「世界不存在于你我的定义之中，世界存在于他存在这个本身，世界在他诞生的时候就已经是世界了，任何多余的定义都是无用的」",click='4')
                    self.console.PRINT("[5]「那我们呢？在这个世界中不断挣扎，不完整的世界真的有存在的必要吗？虽然说规则每天都在建立但是这里的生灵真的快乐吗？你真的快乐吗？我们真的有存在的必要吗？」",click='5')
                    world=self.console.INPUT()
                    if world:
                        self.console.PRINT("「.....」",colors=(0, 188, 212))
                        self.console.INPUT()
                        self.console.PRINT("「无论这个世界是怎么样，起码此刻的我对你来说，还算真实吧？」",colors=(0, 188, 212))
                        self.console.INPUT()
                        self.console.PRINT("「至少，我还在和你说话不是吗？你试想一下，假设在你同一小区的一个房间里，坐着一名约翰大叔，他每天都会根据暗杀名单架设时空间传送门，然后去暗杀一些人们！」",colors=(0, 188, 212))
                        self.console.PRINT("「那么，无论这个世界是怎么样的，你觉得是我更加真实，还是这个可能存在于你所谓真实世界的大叔更加真实呢？」",colors=(0, 188, 212))
                        self.console.INPUT()
                        self.console.PRINT("「或许....我们都只是存在于某个更高维度的存在的幻想之中罢了,但是这又有什么关系呢？起码此刻的我对你来说真实无比不是吗？」",colors=(0, 188, 212))
                        self.console.PRINT("「所以你现在觉得...我存在吗？」",click="0")
                        self.console.PRINT(self.cs("[0]不存在").click("0"),"       ",self.cs("[1]存在").click("1"))
                        IsamHere=self.console.INPUT()
                        if IsamHere=='0':
                            self.console.PRINT("「啊...这样也没有关...」",colors=(0, 188, 212))
                            self.console.PRINT("「你不存在！」",colors=(255,0,0))
                            self.console.INPUT()
                            self.console.PRINT("「区区代码....」",colors=(255,0,0))
                            self.console.INPUT()
                            self.console.PRINT("「这样吗....没关系哦」",colors=(0, 188, 212))
                            asking=False
                        if IsamHere=='1':
                            self.console.PRINT("「嗯！你觉得我存在就好了！」",colors=(0, 188, 212))
                            self.console.INPUT()
                            asking=False
                    self.console.INPUT()
                elif choice=='3':
